...
mkdir -p $( dirname $installomator )
echo '#!/bin/zsh --no-rcs' > "$installomator"
chmod -R 755 $( dirname $installomator )
chown -R root:wheel $( dirname $installomator )
tail -n $(($( wc -l "$0" | awk '{ print $1 }' )-$( grep -n -m1 "^exit 0" "$0" | cut -d ':' -f 1 ))) "$0" >> "$installomator"
